  ...
  return (
    <div className='flex flex-row align-middle gap-[2px]'>
      <Redirect link={homelink} shallow>
        <Image className='dark:svg-filter-dark' src={'/svg/home.svg'} alt='Home' height={27} width={27}/>
      </Redirect>
      {segments.map((segment, idx, arr) => {
        return (
          <div key={segment.label + idx.toString()} className='flex flex-row gap-[2px]'>
            <p className='text-xl'>/</p>
            <Redirect link={homelink + arr.slice(0, idx + 1).reduce((prev, seg) => `${prev}/${seg.linkSegment}`, "")} className='link link-hover font-sans font-semibold text-gray-800 text-xl dark:text-base-dark' shallow>
              {segment.label}
            </Redirect>
          </div>
        )
