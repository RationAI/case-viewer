...
const getCurrentLevelFromHierarchy = (
  hierarchy: CaseHierarchy,
  relativePath: string[],
) => {
  const currLvl = relativePath.reduce((lastLevel, childLevelName) => {
    const invalidPathLevel: CaseHierarchy = {
      levelName: 'invalid path',
      lastLevel: true,
      items: [],
    };
    ...
    const childLevel = (lastLevel.items as CaseHierarchy[]).find(
      (childLevel) => childLevel.levelName === childLevelName,
    );
...
const getTableStructureFromLevel = (
  currLvl: CaseHierarchy,
  relativePath: string[],
) => {
  const tableStructure: TableStructureT = {
    parent:
      currLvl.levelName &&
      `${basePageLink}/path${relativePath.slice(0, relativePath.length - 1).reduce((path, param) => `${path}/${param}`, '')}`,
    folders: !currLvl.lastLevel
      ? (currLvl.items as CaseHierarchy[]).map((item) => {
          return {
            name: item.levelName || '',
      ...
      : undefined,
    cases: currLvl.lastLevel
      ? (currLvl.items as CaseH[]).map((item) => {
          return item;
        })
      : undefined,
    mergeCases: true,
...
export default function CaseHierarchyLevel({ caseHierarchy }: Props) {
  ...
  useEffect(() => {
    const getCurrentLevel = async (hierarchy: CaseHierarchy) => {
      const currLvl = getCurrentLevelFromHierarchy(
        hierarchy,
        getPathParts(relativePath, 3),
      );
  ...
  return (
    <div className="flex flex-col gap-4 p-1">
      {getPathParts(relativePath, 3).length > 0 ? (
        <SegmentedPathLink
          homelink={HIERARCHY_ROOT_PATH}
          segments={getPathParts(relativePath, 3).map((part) => ({
            label: part,
            linkSegment: part,
          }))}
        />
      ...
      {currentLevel ? (
        <Table
          tableStructure={getTableStructureFromLevel(
            currentLevel,
            getPathParts(relativePath, 3),
          )}
        />
