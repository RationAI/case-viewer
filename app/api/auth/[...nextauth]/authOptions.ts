  ...
  providers: [
    {
      ...
      authorization: {
        url: Boolean(process.env.NEXT_AUTH_OIDC_WELL_KNOWN)
          ? undefined
          : process.env.NEXT_AUTH_AUTHORIZATION_ENDPOINT,
        params: { scope: process.env.NEXT_AUTH_OIDC_SCOPE },
      },
      token: Boolean(process.env.NEXT_AUTH_OIDC_WELL_KNOWN)
        ? undefined
        : process.env.NEXT_AUTH_TOKEN_ENDPOINT,
      userinfo: Boolean(process.env.NEXT_AUTH_OIDC_WELL_KNOWN)
        ? undefined
        : process.env.NEXT_AUTH_USER_ENDPOINT,
      idToken: true,
      checks: ['pkce', 'state'],
      profile(profile) {
        return {
