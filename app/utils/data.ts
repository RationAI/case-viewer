...
export const getCaseSlides = async (rootApi: Root, caseId: string) => {
  ...
  const slides = (await rootApi.cases.wsiExplorer.slides(caseId)).filter(
    (slide) => !slide.deleted,
  );
  return slides;
};
...
export const getSlideMetadata = async (rootApi: Root, slideId: string) => {
  const metadata =
    await rootApi.rationai.globalStorage.wsiMetadata.getSlideMetadata(slideId);
  return metadata;
};

export const getSlideVisualizations = async (
  slideId: string,
  rationaiApi: V3.RationAI,
) => {
  let fetchedVis;
  try {
    fetchedVis =
      await rationaiApi.globalStorage.wsiMetadata.getVisualizations(slideId);
  } catch (e) {
    fetchedVis = {};
  }
