...
export const openXOpat = async (visualization: object) => {
  ...
  const useSecureCookies = !process.env.NEXT_PUBLIC_ORIGIN!.includes("localhost");
  const fullHost = new URL(process.env.NEXT_PUBLIC_ORIGIN!).hostname
  const hostName = useSecureCookies ? fullHost.split(".").slice(1).join(".") : 'localhost';

  if(method === 'GET') {
    const newCookie = `_sharedEmpaiaRefreshToken=${session?.refreshToken}; domain=${hostName}; path=/; SameSite=Lax; Secure`;
    document.cookie = newCookie;
    window.open(encodeURI(`${xOpatUrl}/index.php#${JSON.stringify(visualization)}`), "_blank", )
  }
